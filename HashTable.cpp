...
void HashTable::Insert(Bid bid)
{
    ...
    // Calculate the the key of bid
    unsigned key = hash(atoi(bid.bidId.c_str()));

    // retrieve node by using the key
    Node* oldNode = &(nodes.at(key));

    // if not entry found for this key
    if (oldNode == nullptr)
    {
...
Bid HashTable::Search(string bidId)
{
    ...
    // the linked list to find the match
    while (node != nullptr)
    {
